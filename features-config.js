/**
 * Extra feature config panels — nested self-roles + channel fill fix.
 */
(function () {
  "use strict";
    ...
    list.innerHTML = cats
      .map(function (c, ci) {
        var kids = (c.children || [])
          .map(function (ch, chi) {
            ...
            return (
              ...
              mode +
              '</span> <button type="button" data-rm-sub="' +
              ci +
              ":" +
              chi +
        ...
        return (
          ...
          " " +
          (c.name || "Category") +
          '</strong> <button type="button" data-rm-cat="' +
          ci +
    ...
    try {
      ...
      var cats = (cfg.categories || []).map(function (c) {
        ...
        children.push({
          id: Math.random().toString(36).slice(2, 10),
          name: name.slice(0, 100),
          emoji: ($("sr-sub-emoji")?.value || "").trim() || "✨",
          description: ($("sr-sub-desc")?.value || "").trim().slice(0, 500),
          mode: $("sr-sub-mode")?.value === "single" ? "single" : "multi",
          roles: [],
        });
  ...
  function boot() {
    ...
    function tryPatch() {
      ...
      if (typeof window.saveConfig === "function" || typeof saveConfig === "function") {
        ...
        console.log("[features-config] nested self-roles ready");
        return;
      }
