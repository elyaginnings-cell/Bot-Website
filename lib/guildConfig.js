...
function normalizeApplications(patchApp, currentApp) {
  ...
  // Keep pending only on bot; website does not own it
  if (out.pending && typeof out.pending === "object" && !p.pending) {
    // leave as-is from current if present
  }
...
export function preferWebsiteShop(websiteConfig, botConfig) {
  ...
  for (const key of ["bump", "verification", "suggestions", "tickets", "qotd", "currency", "birthday", "selfRoles", "analytics", "ai", "automod", "applications"]) {
