...
import { normalizeLoa } from "./loaConfig.js";
import { normalizeActivityCheck } from "./activityCheckConfig.js";
...
function normalizeApplications(patchApp, currentApp) {
  const cur = currentApp && typeof currentApp === "object" ? currentApp : {};
  const p = patchApp && typeof patchApp === "object" ? patchApp : {};
  const out = { ...cur, ...p };

  if (p.enabled !== undefined) out.enabled = !!p.enabled;
  if (p.reviewChannelId !== undefined) out.reviewChannelId = p.reviewChannelId || null;
  if (p.reviewRoleId !== undefined) out.reviewRoleId = p.reviewRoleId || null;
  if (p.announcementsChannelId !== undefined) out.announcementsChannelId = p.announcementsChannelId || null;
  if (p.panelChannelId !== undefined) out.panelChannelId = p.panelChannelId || null;
  if (p.buttonLabel !== undefined) out.buttonLabel = String(p.buttonLabel || "Apply for Staff").slice(0, 80);
  if (p.embedTitle !== undefined) out.embedTitle = String(p.embedTitle || "Staff Applications").slice(0, 256);
  if (p.embedDescription !== undefined) out.embedDescription = String(p.embedDescription || "").slice(0, 4000);
  if (p.acceptMessage !== undefined) out.acceptMessage = String(p.acceptMessage || "").slice(0, 2000);

  if (Array.isArray(p.positions)) {
    out.positions = p.positions
      .filter((x) => x && x.name && x.roleId)
      .slice(0, 25)
      .map((x) => ({
        id: String(x.id || crypto.randomBytes(3).toString("hex")),
        name: String(x.name).slice(0, 100),
        roleId: String(x.roleId),
        linkedRoleIds: Array.isArray(x.linkedRoleIds) ? x.linkedRoleIds.map(String).slice(0, 10) : [],
        description: x.description ? String(x.description).slice(0, 100) : "",
      }));
  } else if (!Array.isArray(out.positions)) {
    out.positions = Array.isArray(cur.positions) ? cur.positions : [];
  }
  ...
  if (Array.isArray(p.questions)) {
    out.questions = p.questions
      ...
      .slice(0, 5)
      .map((q) => ({
        id: String(q.id || crypto.randomBytes(3).toString("hex")).slice(0, 100),
        label: String(q.label).slice(0, 45),
        placeholder: q.placeholder ? String(q.placeholder).slice(0, 100) : "",
        required: q.required !== false,
        maxLength: Math.min(Number(q.maxLength) || 500, 1000),
      }));
  } else if (!Array.isArray(out.questions)) {
    out.questions = Array.isArray(cur.questions) ? cur.questions : [];
  }
...
export async function mergeGuildConfig(guildId, patch) {
  ...
  if (patch.automod && typeof patch.automod === "object") next.automod = { ...(current.automod || {}), ...patch.automod };

  if (patch.ai && typeof patch.ai === "object") {
    next.ai = normalizeAi(patch.ai, current.ai);
  }

  if (patch.applications && typeof patch.applications === "object") {
    next.applications = normalizeApplications(patch.applications, current.applications);
  }

  if (patch.loa && typeof patch.loa === "object") {
    next.loa = normalizeLoa(patch.loa, current.loa);
  }

  if (patch.activityCheck && typeof patch.activityCheck === "object") {
    next.activityCheck = normalizeActivityCheck(patch.activityCheck, current.activityCheck);
  }
...
export function preferWebsiteShop(websiteConfig, botConfig) {
  ...
  for (const key of ["bump", "verification", "suggestions", "tickets", "qotd", "currency", "birthday", "selfRoles", "analytics", "ai", "automod", "applications", "loa", "activityCheck"]) {
