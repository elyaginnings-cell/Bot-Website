...
async function loadServers() {
  ...
  try {
    ...
    if (!response.ok) {
      const msg = data.error || data.code || `HTTP ${response.status}`;
      list.innerHTML = `<div class="server-item"><span>❌ ${msg}</span></div>`;
      return;
    }
...
function fillChannelSelects() {
  ["warn-channel", "invite-channel", "dashboard-log-channel", "level-up-channel"].forEach((id) => {
    ...
    const emptyLabel =
      id === "level-up-channel" ? "Same channel they leveled in" : "Select a channel...";
...
function renderShopItems() {
  ...
  list.innerHTML = items
    .map((item, i) => {
      ...
      const extra = [];
      if (item.roleId) extra.push(role ? role.name : item.roleId);
      if (item.durationHours) extra.push(`${item.durationHours}h`);
      if (item.amount) extra.push(`amt ${item.amount}`);
      if (item.titleText) extra.push(`"${item.titleText}"`);
      return `<div class="level-role-row"><span><strong>${item.name}</strong> — ${item.price} ☕ (${typePretty(item.type)}${extra.length ? ": " + extra.join(", ") : ""})</span>
...
async function saveModeration() {
  ...
  try {
    const data = await saveConfig({ warnChannelId: channelId });
...
async function saveInvites() {
  ...
  try {
    const data = await saveConfig({ inviteLeaderboardChannelId: channelId });
