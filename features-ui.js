  ...
  function inject() {
    ...
    if (currencySection && !$("cur-weekly-min")) {
      ...
      extras.innerHTML =
        ...
        '<div class="input-group"><label>Weekly min</label><input id="cur-weekly-min" type="number" min="0" value="200"></div>' +
        '<div class="input-group"><label>Weekly max</label><input id="cur-weekly-max" type="number" min="0" value="500"></div></div>' +
        '<h3 class="subhead">Work</h3><div class="config-grid">' +
        '<div class="input-group"><label>Work min</label><input id="cur-work-min" type="number" min="0" value="50"></div>' +
        '<div class="input-group"><label>Work max</label><input id="cur-work-max" type="number" min="0" value="150"></div>' +
        '<div class="input-group"><label>Work cooldown (min)</label><input id="cur-work-cd" type="number" min="1" value="60"></div></div>';
      if (saveBtn && saveBtn.parentNode) saveBtn.parentNode.insertBefore(extras, saveBtn);
      else currencySection.appendChild(extras);
    }

    if (!$("bump")) {
      var html =
        '<section id="bump" class="page-section"><div class="card form-card wide">' +
        '<span class="eyebrow">BUMP</span><h2>Bump rewards</h2>' +
        '<p class="form-hint">Rewards members when Disboard (or similar) is bumped.</p>' +
        '<label class="toggle"><input type="checkbox" id="bump-enabled" checked> <span>Enabled</span></label>' +
        '<div class="config-grid">' +
        '<div class="input-group"><label>Reward min</label><input id="bump-reward-min" type="number" min="0" value="50"></div>' +
        '<div class="input-group"><label>Reward max</label><input id="bump-reward-max" type="number" min="0" value="150"></div>' +
        '<div class="input-group"><label>Cooldownoldown (minutes)</label><input id="bump-cooldown" type="number" min="1" value="110"></div></div>' +
        ...
        '<span class="eyebrow">VERIFICATION</span><h2>Verification</h2>' +
        '<p class="form-hint">Members click a button to get a role.</p>' +
        ...
        '<div class="input-group"><label>Role to grant</label><select id="verify-role"><option value="">Select a role...</option></select></div>' +
        '<div class="input-group"><label>Log channel</label><select id="verify-log-channel"><option value="">None</option></select></div>' +
        '<div class="input-group"><label>Button label</label><input id="verify-btn-label" type="text" value="Verify" maxlength="80"></div>' +
        '<div class="input-group"><label>Embed title</label><input id="verify-title" type="text" value="Verification" maxlength="256"></div>' +
        '<div class="input-group" style="grid-column:1/-1"><label>Embed description</label><textarea id="verify-desc" rows="2" maxlength="2000">Press the button below to gain access to the server.</textarea></div></div>' +
        ...
        '<span class="eyebrow">SUGGESTIONS</span><h2>Suggestions</h2>' +
        '<p class="form-hint">Members use /suggest. Staff review in a private channel.</p>' +
        '<label class="toggle"><input type="checkbox" id="suggest-enabled" checked> <span>Enabled</span></label>' +
        '<div class="config-grid">' +
        '<div class="input-group"><label>Public channel</label><select id="suggest-channel"><option value="">Select a channel...</option></select></div>' +
        ...
        '<div class="input-group"><label>Ping role on new</label><select id="suggest-ping-role"><option value="">None</option></select></div></div>' +
        ...
        '<section id="tickets" class="page-section"><div class="card form-card wide" id="tickets-panel-host" data-tickets-host="1">' +
        '<span class="eyebrow">TICKETS</span><h2>Support tickets</h2>' +
        '<p class="form-hint" id="tickets-loading-hint">Loading full ticket settings… open this tab again or hard-refresh if stuck.</p>' +
        '<div id="tickets-legacy-fallback">' +
        ...
        '<p class="form-hint">Posts <strong>one button</strong> in Discord. Members click it → private menu of categories → sub-categories → roles.</p>' +
        ...
        '<div class="input-group"><label>Name</label><input id="sr-cat-name" type="text" placeholder="e.g. Games" maxlength="100"></div>' +
        '<div class="input-group" style="grid-column:1/-1"><label>Description</label><input id="sr-cat-desc" type="text" placeholder="Optional" maxlength="500"></div>' +
        '</div>' +
        '<button class="button" id="sr-add-cat" type="button">Add top category</button>' +
        '<h3 class="subhead">2 · Sub-category</h3><div class="config-grid">' +
        '<div class="input-group"><label>Under category</label><select id="sr-parent-cat"><option value="">Select…</option></select></div>' +
    ...
    console.log("[features-ui] injected (tickets host + panels)");
    return true;
  }
